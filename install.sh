...
declare -A repos
repos["degoogle_hunter"]="ranjanbug/degoogle_hunter"
...
printf "\n\n${bgreen}#######################################################################${reset}\n"
printf "${bgreen} reconWEB installer/updater script ${reset}\n\n"
...
if [[ $(id -u | grep -o '^0$') == "0" ]]; then
    ...
    if sudo -n false 2>/dev/null; then
        ...
        printf "${bred} echo \"${USERNAME}  ALL=(ALL:ALL) NOPASSWD: ALL\" > /etc/sudoers.d/reconWEB${reset}\n\n"
    fi
    SUDO="sudo"
fi
...
printf "${bblue} Running: Looking for new reconWEB version${reset}\n\n"
...
then
    ...
    if [ -n "$(git status --porcelain | egrep 'reconweb.cfg$')" ]; then
        mv reconweb.cfg reconweb.cfg_bck
        printf "${yellow} reconweb.cfg has been backed up in reconweb.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} reconWEB is already up to date!${reset}\n\n"
fi
...
eval wget -nc -O ~/.config/notify/provider-config.yaml https://gist.githubusercontent.com/ranjanbug/23a996bca189a11e88251367e6583053/raw $DEBUG_STD
...
eval wget -O resolvers_trusted.txt https://gist.githubusercontent.com/ranjanbug/ae9ed7e5c786461868abd3f2344401b6/raw $DEBUG_STD
eval wget -O subdomains.txt https://gist.github.com/ranjanbug/a307a04a222fab5a57466c51e1569acf/raw $DEBUG_STD
eval wget -O permutations_list.txt https://gist.github.com/ranjanbug/ffc2b14d283e8f8eff6ac83e20a3c4b4/raw $DEBUG_STD
eval wget -nc -O fuzz_wordlist.txt https://raw.githubusercontent.com/ranjanbug/OneListForAll/main/onelistforallmicro.txt $DEBUG_STD
eval wget -O lfi_wordlist.txt https://gist.githubusercontent.com/ranjanbug/a89a0c7861d49bb61a09822d272d5395/raw $DEBUG_STD
eval wget -O ssti_wordlist.txt https://gist.githubusercontent.com/ranjanbug/ab5277b11da7369bf4e9db72b49ad3c1/raw $DEBUG_STD
eval wget -O headers_inject.txt https://gist.github.com/ranjanbug/d62ab8f8ffd28e1c206d401081d977ae/raw $DEBUG_STD
eval wget -O axiom_config.sh https://gist.githubusercontent.com/ranjanbug/6e2d9f4932fd38d84610eb851014b26e/raw $DEBUG_STD
